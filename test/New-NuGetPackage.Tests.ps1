...
Describe "New-NugetPackage PS$PSVersion with commandline parameter only" {
    ...
    Context 'the nuspec read from the package with Get-NuGetPackageNuspec' {
        BeforeAll {
            $nuspec = Get-NuGetPackageNuspec -Path $nupkgPath
        }
    ...
    Context "The expanded package" {
        It 'should contain exactly the files provided on the command line' {
            $dst = Join-Path "$packageDir" "$id.$version"
            Expand-Archive $nupkgPath -DestinationPath $dst
