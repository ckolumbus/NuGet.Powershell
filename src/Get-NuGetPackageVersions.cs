...
{
    [Cmdlet(VerbsCommon.Get, "NuGetPackageVersions")]
    [OutputType(typeof(PackageIdentity))]
    public class GetNugetPackageVersionsCmdlet : AsyncCmdlet
    {
        [Parameter(
            Mandatory = true,
            Position = 0,
        ...
        protected override async Task ProcessRecordAsync()
        {
            ...
            {
                var result = await Helpers.FindPackageVersionsAsync(id, nuGetFramework, repositories, cache, logger: this);
                foreach (var r in result)
                {
                    WriteObject(new PackageIdentity(id, r));
                }
            }
        }
    }
